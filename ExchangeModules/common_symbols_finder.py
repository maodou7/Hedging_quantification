...
class CommonSymbolsFinder:
    ...
    def __init__(self, exchange_instance: ExchangeInstance, market_processor: MarketProcessor, config: dict):
        ...
        # 获取第一个交易所的配置作为基准
        first_exchange_config = next(iter(config.values()))
        self.market_types = first_exchange_config['market_types']
        self.quote_currencies = first_exchange_config['quote_currencies']
    ...
    def _initialize_common_symbols(self):
        ...
        """
        for market_type in self.market_processor.get_enabled_market_types(self.market_types):
            self.common_symbols[market_type] = {quote: set() for quote in self.quote_currencies}
    ...
    def _get_empty_market_sets(self) -> Dict[str, Dict[str, Set[str]]]:
        ...
        """
        return {mtype: {quote: set() for quote in self.quote_currencies}
                for mtype in self.market_processor.get_enabled_market_types(self.market_types)}
